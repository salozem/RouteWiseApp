...
def generateAdjacencyList(listPath, streetPath):
    ...
    def trafficHour(time):
        fact = round(5 * math.cos(2 * (time / 3) - 1)
                     * math.sin((time / 6) - 2) + 5, 3)
        if fact > 10:
            return 10
        if fact < 0:
            return 0
    ...
    def getWeight(pos1, pos2):
        lima_street = street[(pos1, pos2)]
        val = lima_street["val"]
        length = lima_street["length"]
    ...
    def newAdjacencyList():
        ...
        for key, arr in adjacencyList:
            newAdjacencyList.append(
                list(map(lambda x: (keyValue[x], getWeight(key, x)), arr)))
